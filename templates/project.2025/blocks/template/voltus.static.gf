...
#
################################################################################
# Filename: templates/project.2025/blocks/template/voltus.static.gf
# Purpose:  Batch static power and rail analysis flow
################################################################################
...
gf_add_tool_commands '
    ...
    # set_power_lib_files [gconfig::get_files lib -view $STATIC_POWER_VIEW]
    read_libs [gconfig::get_files lib -view $STATIC_POWER_VIEW]
    ...
    # Read constraints    
    read_sdc [gconfig::get_files sdc -view $STATIC_POWER_VIEW]

    # Read design timing files
    if {[llength $TWF_OUT_DIR]} {
        set view [gconfig::get analysis_view_name -view $STATIC_POWER_VIEW]
        ...
        foreach file [gconfig::get_files twf -view $STATIC_POWER_VIEW] {
    ...
    foreach file [gconfig::get_files spef -view $STATIC_POWER_VIEW] {lappend files $file}
    foreach file $files {
        if {[file exists $file]} {
            puts "SPEF file: $file"
        } else {
    ...
    set VOLTUS_PGV_LIBS [join [concat {`$VOLTUS_PGV_LIBS`} [gconfig::get_files pgv -view $STATIC_RAIL_VIEW]]]
