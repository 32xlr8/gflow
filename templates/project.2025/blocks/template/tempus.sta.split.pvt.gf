...
#
################################################################################
# Filename: templates/project.2025/blocks/template/tempus.sta.split.pvt.gf
# Purpose:  Batch signoff STA flow with several tasks splitted by delay corners
################################################################################
...
gf_add_tool_commands -file "./tasks/$TASK_NAME/run.tcl" '
    ...
    foreach mask $MMMC_VIEWS {
        set group [join [list [lindex $mask 1] [lindex $mask 2] [lindex $mask 3] [lindex $mask 4]] ""]
